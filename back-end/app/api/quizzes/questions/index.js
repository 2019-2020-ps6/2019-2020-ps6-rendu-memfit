...
router.post('/', (req, res) => {
  try {
    ...
    let question
    if (req.body.image) {
      question = Question.create({ statement: req.body.statement, image: req.body.image, quizId })
    } else {
      question = Question.create({ statement: req.body.statement, quizId })
    }
...
router.put('/:questionId', (req, res) => {
  try {
    const question = getQuestionFromQuiz(req.params.quizId, req.params.questionId)
    const updatedQuestion = Question.update(req.params.questionId, { statement: req.body.statement, quizId: question.quizId })
